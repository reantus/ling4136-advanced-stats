...
my_priors <-  c(prior(normal(6.5, 1), class = 'Intercept'),
               prior(normal(0, 0.1), class = 'b', coef = 'log_frequency'))
...
xmdl <- brm(log_rt ~ log_frequency,
            family = gaussian(),
            file = "../models/05_wide_prior_mdl.rds",
            prior = my_priors,
            data = quantese,
            chains = 4,
            cores = 4, 
...
spread_draws(b_Intercept, sigma) %>% 
  ...
  mutate(intercept = b_Intercept,
         prediction = b_Intercept + sigma) %>% 
  # and then bring into a long format like the data
  select(intercept, prediction) %>% 
  pivot_longer(everything(), names_to = "log_frequency", values_to = "log_rt") %>% 
  # plot
  ggplot(aes(x = log_rt, fill = log_frequency)) +
    geom_density(alpha = 0.5) +
  stat_pointinterval(aes(y = 0.05), .width = 0.95) +
  ylim(0, 0.2) +
  labs(y = 'prediction')
plot
```
